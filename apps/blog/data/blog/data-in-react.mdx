...
但那也只是针对简单应用，组件层级一两层的情况下。组件层级多了，继续这么做，那会是你的噩梦。当然也可以结合 `Context` 来进行状态提升，那样不会有一层层传递 `props` 的噩梦，但会有另一个噩梦。

2.**数据修改引起的额外 rerender **。按照上面的方法，使用 `Context` 结合 `useState` ，把数据放在顶层进行共享。在复杂应用里，会带来严重的性能问题。React 的渲染机制是，一旦触发了渲染，从顶层节点开始，遍历整颗树，
找到数据变动的节点，然后从这个节点以下所有的子节点，都会重新渲染一次。而你可能仅仅只是改动了其中某一个子节点的数据，只是想重新渲染这一个子节点而已。所有其它节点，都是陪跑的，造成了大量不必要的渲染，和性能的浪费。
但问题就在这里，你想共享数据的组件越多，你状态提升的层级就会越高，层级越高，导致的渲染范围就越广。所以这种方式一般只用于共享一下全局的不怎么会变化的数据。
