...
updatedAt: '2022-06-11 08:46:06'
advice: true
summary: redux 理念真的很简单，实现也很简单，最纯粹的东西往往都很简单。
---
...
使用 subscribe 订阅 store 的更新

在订阅更新的 callback 中：

+ 使用 getState 获取当前 state
+ 从完整的 state 中取出当前UI需要的数据
+ 使用取到的数据更新 UI

如果必要，初始化的时候使用 initialState 更新 UI

派发 Redux Action 来响应 UI 的输入

抛开其它暂时不管，只关心核心的 store，最基础的实现：
...
![redux_flow.png](https://zackdkblog.oss-cn-beijing.aliyuncs.com/redux_flow.png)
...
![middleware-flow.png](https://zackdkblog.oss-cn-beijing.aliyuncs.com/middleware-flow.png)
...
redux-thunk ，就是特别简单的一个，简单判断了下action是否是函数，是函数就调用，不是就传递Action。
代码如下：
...
相较之下，redux-saga 和 redux-observable就复杂太多了。
...
-网上觉得很有意思的观点-
Store 扁平化有很大原因是 js 对 immutable 支持力度不够，导致对深层数据修改非常麻烦导致的。
